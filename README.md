...
- Installation and configuration of a **mandatory access control** system (AppArmor)
...
1. **Partitioning:** Used **LVM (Logical Volume Manager)** with manual partitioning to create separate mounts for `/`, `/home` and `/boot`. This provides flexibility for resizing partitions later and follows good practice for isolating directories with high write activity.

2. **Security Policies:**
   - Implemented a strong password policy via `/etc/login.defs`
   - Configured **sudo** with a dedicated group (`sudo`) and restricted privileges in `/etc/sudoers.d/`
   - Set up **cron jobs** for regular system updates and monitoring script execution

3. **User Management:** Created a user with low privileges and added them to the sudo group.
...
|---------|----------------------------------|---------------|
| **Complexity** | Simpler, designed for ease of use | More feature-rich, designed for complex network environments |
...
### Installation & Setup
1. **Create VM:** Allocate minimal resources (e.g. 1024MB RAM, 8GB dynamic disk)
...
3. **Post-Install Configuration:**
   ...
   - Install required packages: `sudo`, `ufw`, `apparmor` (`vim` if wanted)
   - Configure the password policy in `/etc/login.defs`, `/etc/sudoers.d/sudo_config` and `/etc/pam.d/common-password`
...
### Execution
- Connect via SSH: `ssh user@localhost -p 4241`
- The monitoring script can be executed manually: `sudo sh /home/user/monitoring.sh`
...
- **Command & Syntax Help:** For finding correct command-line options or configuration file syntax
